...
export const RISK = {
  ...
  // ★ 느슨하게 — 몰래 다니는 것 자체는 즐길 수 있어야 한다("가혹하되 좌절 아닌"). 흔적은
  //   매번 조금(+1)씩만 쌓이므로, 문턱을 높여 **집착적으로 남용**할 때에만 닿게 한다.
  //   가끔 나가는 관리형은 경고에도 닿지 않는다.
  /** 경고 서술이 뜨는 문턱("섭정공이 지켜보고 있소"). */
  tutorWarn: 12,
  /** 해고 위기 이벤트가 뜨는 문턱(느슨하게 높게 — 집착적 남용에만). */
  tutorDead: 25,
} as const
...
export const INITIAL_RESOURCES = {
   ...
   * ★ 20 → 0 (UI 리디자인 2단계).
   *
   *   20 은 구간표에서 "가늠함"이다 — 열한 살짜리를 섭정공이 벌써 가늠한다는 뜻이 되어
   *   이 게임의 출발점과 어긋났다. 0 은 "아이로만 봄"이고 그게 1년차의 진실이다.
   *
   *   상한과도 맞물린다: 15세 전 상한이 30인데 20 에서 시작하면 4년 내내 10점만 움직여
   *   상한이 사실상 없는 규칙이었다. 0 이면 30점 여유가 생겨
   *   "어릴 땐 아무리 잘해도 30까지"가 실제로 작동한다.
   */
  regentRapport: 0,
  /** 11세 허수아비에서 출발한다. */
  courtInfluence: 10,
