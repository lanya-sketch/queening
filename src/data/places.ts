...
export const CHAPEL = {
  ...
  // ★ [9-A] 신앙 깊은 왕이 오면 사제들이 우러러 맞는다(비-성물 쓰임 — 사제 태도).
  devoutTone:
    '사제들이 왕을 알아보고 깊이 예를 표했다. 독실한 군주가 성물 앞에 서는 것을, 그들은 예사롭지 않게 여겼다.',
  // ★ [9-A] 신앙이 얕아 성검이 본산으로 봉송된 뒤 — 있어야 할 자리가 비어 있음을 보여준다.
  swordGone:
    '제단 위는 비어 있었다. 성검이 놓였던 자리에 촛농만 굳어 있을 뿐, 그 검도 그것을 든 자도 이제 이 궁에 오지 않는다.',
} as const
