...
export const DEAD_END_SCENES: Record<string, { title: string; lines: SceneLine[] }> = {
  ...
  // ★ [3] 넷째 축 — 이겼다가 무너진 것. 앞의 셋은 못 이룬 것이고, 이건 실권을 쥐고도 못 지킨 것.
  폐위: {
    title: '지키지 못한 옥좌',
    lines: [
      N('실권은 손에 넣었다. 아홉 해를 밀어붙여, 마침내 제 이름으로 나라를 움직였다.'),
      N('그러나 밀려난 자를 끝내 달래지도, 억누르지도 못했다. 의심은 반란이 되어 돌아왔다.'),
      N('그 밤, 열린 성문 안으로 옛 문장을 단 병사들이 들었고, 지켜 줄 것이 없었다.'),
      N('힘으로 오른 자리는 힘으로 지켜야 했는데, 그 힘을 홀로 다 쥐려다 아무도 남기지 못했다.'),
      N('가장 높이 올랐던 왕이, 가장 멀리 떨어졌다.'),
    ],
  },
  암살: {
    title: '달빛만이 본 밤',
    lines: [
      N('실권은 손에 넣었다. 밀려난 자의 처소에 밤마다 사람이 들던 것을, 알면서도 그저 두었다.'),
      N('반란이 판을 갖추기도 전에, 더 조용한 칼이 먼저 왔다.'),
      N('막을 손도, 낌새를 읽을 눈도, 앞을 가려 줄 사람도 그 밤에는 없었다.'),
      N('무예든 사람이든, 무엇 하나라도 곁에 두었다면 넘겼을 밤이었다. 아무것도 없었다.'),
      N('옥좌는 왕을 앉혔으나, 왕을 지켜 주지는 않았다.'),
    ],
  },
}
