 ...
 * 암살 미스터리 회수 지점.
 *
 * 진실은 코드에 고정되어 있다 — 설계자는 모후, 공모자는 그의 오라비인 섭정,
 * 동기는 선황의 급진 정치. 플레이어는 그 진실에 "도달"할 뿐 바꾸지 못한다.
 *
 * 도달 깊이는 두 단계다.
...
export const TRUTH_EVENTS: GameEvent[] = [
  {
    ...
    title: '덮인 밤',
    text:
      '봉함된 진맥 기록과, 여섯 달 동안 네 배로 뛴 명목 없는 지출.\n' +
      '숫자는 사람보다 정직하다. 돈이 어디로 갔는지 따라가면 누가 무엇을 ' +
      '해주었는지가 나온다 — 그 여섯 달에 값을 치른 항목이 스물셋이고, ' +
      '그중 아홉이 선황의 침전에 드나들 수 있는 자리였다.\n' +
      '기록을 덮는 데에도, 입을 막는 데에도 값이 든다. 그 값이 여기 적혀 있다.\n' +
      '선황은 병으로 돌아가시지 않았다. 그리고 그 뒤를 덮은 것은 섭정과 ' +
      '귀족파의 손이다.\n' +
      '군주는 오래 아무 말도 하지 않다가, 아주 조용히 물었다.\n' +
      '"스승님은 언제부터 아셨습니까."',
    // 재정 특화 빌드의 대체 경로.
    ...
    choices: [
      {
        ...
        resultText:
          ...
          '내가 안다는 걸 몰라야 합니다."\n' +
          '군주는 고개를 끄덕였다. 그날 이후 아이는 섭정 앞에서 조금 더 ' +
      ...
      },
      {
        ...
        resultText:
          '군주는 남쪽 역참의 환관 이야기를 지나가듯 꺼냈다. 섭정은 눈 하나 ' +
  ...
  },
  {
    ...
    title: '어머니의 글씨',
    text:
      '내명부의 재고에서 나온 약. 두 달 전부터 시작된 남매의 서찰. ' +
      '그리고 반포되지 못한 조칙 — 토지를 나누고, 사병을 거두고, 세습을 끊는다.\n' +
      '모후의 친정은 그 조칙이 반포되면 사라질 가문이었다.\n' +
      '섭정은 손이었다. 손을 움직인 것은 그의 누이다.\n' +
      '\n' +
      '군주는 서찰 한 통을 끝까지 읽고, 다시 처음부터 읽었다. 열두 살에 ' +
      '받았던 다과상이 그 방에 다시 놓인 것처럼.\n' +
      '"…어마마마의 글씨입니다."\n' +
    ...
    choices: [
      {
        ...
        resultText:
          '군주는 서찰을 원래 자리에 돌려놓게 했다.\n' +
          '이듬해 문안에서 모후는 여전히 아들의 소맷단을 나무랐고, 군주는 ' +
          '여전히 예를 갖추었다. 아무것도 달라지지 않은 한 시진이었다.\n' +
