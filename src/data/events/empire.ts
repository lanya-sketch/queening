 ...
 *
 * ★ [9-C1] 참칭 전쟁 — empire_claimed 뒤 20세에 systems/turn.ts 가 warOutcome 으로 승/패를 굳히고
 *   (war_won/war_lost), 아래 두 이벤트가 그 결과를 서사로 드러낸다(9년의 클라이맥스). 명분별 결은
 *   엔딩 삽입이 가른다("하늘이 인정한 황제"/"교황이 관을 씌운 황제"/"칼로 선 황제"/패배).
 */
export const EMPIRE_EVENTS: GameEvent[] = [
  ...
  {
    // ★★ [9-C1] 참칭 전쟁 — 승리. turn.ts 가 war_won 을 굳힌 다음 달, 전쟁을 서사로 드러낸다.
    id: 'war-victory',
    title: '국경의 전쟁',
    text:
      '제국의 군대가 국경을 넘었다. 아홉 해 전 이 나라를 손바닥처럼 내려다보던, 바로 그 군대였다.\n' +
      '그러나 국경에 선 것은 열한 살에 옥좌 앞에서 떨던 아이가 아니었다. 아홉 해가 그 아이를 ' +
      '왕으로, 이제 스스로 황제라 칭한 자로 세웠다.\n' +
      '싸움은 길고 참혹했다. 그리고 끝났을 때, 진창에 꺾인 것은 제국의 깃발이었다.\n' +
      '무너지던 상국은, 제가 남긴 마지막 왕관 앞에서 마저 무너졌다. 국경 너머까지, 새 황제의 이름이 닿았다.',
    condition: { minAge: 20, flags: { war_won: true } },
    once: true,
    priority: 43.2,
  },

  {
    // ★★ [9-C1] 참칭 전쟁 — 패배. 다 걸고 잃은 파국(엔딩 tier '배드:참칭실패'로 이어진다).
    id: 'war-defeat',
    title: '국경의 전쟁',
    text:
      '제국의 군대가 국경을 넘었다. 아홉 해 전 이 나라를 손바닥처럼 내려다보던, 바로 그 군대였다.\n' +
      '무너지고 있다 하나 제국은 여전히 대국이었고, 황제를 자칭한 작은 나라를 그냥 두지 않았다.\n' +
      '국경에서 왕의 군대가 부서졌다. 명분도, 세력도, 스스로 칭한 관을 지킬 만큼은 되지 못했다.\n' +
      '다 걸었고, 다 잃었다. 황제를 칭한 값은, 왕으로 남는 것조차 잃는 것이었다.',
    condition: { minAge: 20, flags: { war_lost: true } },
    once: true,
    priority: 43.3,
  },
]
