...
function truthLine(flags: Record<string, boolean>): { title: string; body: string } {
  if (flags.truth_mother_mastermind) {
    return {
      title: '진실 — 설계자까지',
      body:
        '왕은 아버지를 죽인 손과, 그 손을 움직인 사람을 모두 알고 있다. ' +
  ...
  return {
    ...
    body:
      ...
      '왕은 통치를 배웠지만, 자신이 왜 이 자리에 앉았는지는 끝내 알지 못했다.',
  }
}
...
function influenceLine(influence: number): string {
  if (influence >= 70) {
    return (
      '정무는 왕의 손에 있다. 이제 누구의 서명도 필요하지 않다. ' +
      '제국의 사절이 이 궁을 얕보고 돌아간 것이 벌써 아홉 해 전이다.'
    )
  }
  if (influence >= 45) return '궁정의 절반은 왕을 본다. 나머지 절반은 아직 숙부를 본다.'
  if (influence >= 20) return '왕의 이름으로 나가는 문서가 늘었다. 아직 많지는 않다.'
  return '정무는 여전히 섭정공의 손에 있다. 왕은 옥좌에 앉아 있을 뿐이다.'
}

function regentLine(flags: Record<string, boolean>): string {
  if (flags.regent_hostile) return '섭정공과는 끝내 갈라섰다. 남은 것은 힘겨루기뿐이다.'
  if (flags.regent_alliance) return '섭정공은 왕의 손을 잡았다. 그가 무엇을 했는지 알면서도.'
  if (flags.regent_won_over) return '섭정공은 왕을 인정했다. 다만 아직 손은 잡지 않았다.'
  ...
  return (
    <div className="pb-28 lg:pb-6">
      ...
      <section className="mb-4 rounded-xl border border-amber-900/60 bg-amber-950/20 p-4">
        ...
        <p className="mt-2 text-sm leading-relaxed text-amber-100/90">{truth.body}</p>
      ...
      <section className="mb-4 rounded-xl border border-slate-800 bg-slate-900/60 p-4">
        ...
        <p className="mt-2 text-sm leading-relaxed text-slate-400">{influenceLine(game.courtInfluence)}</p>
      </section>

      <section className="mb-4 rounded-xl border border-slate-800 bg-slate-900/60 p-4">
        <h2 className="mb-2 text-sm font-medium text-slate-300">섭정</h2>
        <p className="text-sm leading-relaxed text-slate-400">{regentLine(game.flags)}</p>
      </section>

      <section className="mb-4 rounded-xl border border-slate-800 bg-slate-900/60 p-4">
        <h2 className="mb-3 text-sm font-medium text-slate-300">스무 살의 왕</h2>
