...
function truthLine(flags: Record<string, boolean>): { title: string; body: string } {
  if (flags.truth_mother_mastermind) {
    return {
      title: '진실 — 설계자까지',
      body:
        '군주는 아버지를 죽인 손과, 그 손을 움직인 사람을 모두 알고 있다. ' +
  ...
  if (flags.truth_regent_involved) {
    return {
      title: '진실 — 덮은 손까지',
      body:
        '선황이 병으로 죽지 않았다는 것, 그리고 그 뒤를 덮은 것이 섭정과 귀족파라는 것까지. ' +
  ...
  return {
    ...
    body:
      ...
      '군주는 통치를 배웠지만, 자신이 왜 이 자리에 앉았는지는 끝내 알지 못했다.',
  }
}

function regentLine(flags: Record<string, boolean>): string {
  if (flags.regent_hostile) return '섭정과는 끝내 갈라섰다. 남은 것은 힘겨루기뿐이다.'
  if (flags.regent_alliance) return '섭정은 군주의 손을 잡았다. 그가 무엇을 했는지 알면서도.'
  if (flags.regent_won_over) return '섭정은 군주를 인정했다. 다만 아직 손은 잡지 않았다.'
  return '섭정과의 관계는 아직 어느 쪽으로도 기울지 않았다.'
}
    ...
    <div className="pb-28 lg:pb-6">
      <header className="mb-4">
        ...
        <h1 className="text-xl font-semibold text-amber-100">
          군주는 {GAME_CONFIG.endAge}세가 되었다
        </h1>
        <p className="mt-2 text-sm leading-relaxed text-slate-300">
          9년이 지났다. 열한 살에 어좌에 앉았던 아이는 이제 혼자 설 수 있다.
      ...
      <section className="mb-4 rounded-xl border border-slate-800 bg-slate-900/60 p-4">
        <h2 className="mb-3 text-sm font-medium text-slate-300">스무 살의 군주</h2>
