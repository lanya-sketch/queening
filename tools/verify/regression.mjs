...
for (let i = 0; i < 22 && !fired; i++) {
  ...
  await page.waitForFunction(() => {
    // ★ 화면 훅으로 판별한다 — 문구를 고칠 때마다 이 대기가 조용히 깨지지 않도록.
    if (document.querySelector('[data-event-title]')) return true
    return Boolean(document.querySelector('[data-screen]'))
  }, { timeout: 8000 }).catch(() => {})
...
log('E2 제목:', await apage.locator('[data-event-title]').innerText(),
  ok((await apage.locator('[data-event-title]').innerText()) === '변경의 불빛'))
log('E3 라벨이 "국정 현안":',
