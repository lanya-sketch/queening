...
const aiButton = page.getByRole('button', { name: /AI 설정/ })
log('A2 AI 설정 버튼 상태:', await aiButton.innerText(),
  ok((await aiButton.innerText()).includes('꺼짐')))
...
log('C5 새로고침 후에도 유지:',
  ok((await page.getByRole('button', { name: /AI 설정/ }).innerText()).includes('켜짐')))

log('')
log('=== D. ★ 브라우저 직접 호출 (CORS/헤더) ===')
await page.getByRole('button', { name: /AI 설정/ }).click()
await page.waitForTimeout(250)
const d2 = page.getByRole('dialog', { name: 'AI 설정' })
...
log('E3 삭제 후 AI 꺼짐 표시:',
  ok((await page.getByRole('button', { name: /AI 설정/ }).innerText()).includes('꺼짐')))
